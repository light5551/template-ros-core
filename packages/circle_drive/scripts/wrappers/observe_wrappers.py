...
class LastPictureObsWrapper(gym.ObservationWrapper):
    ...
    def observation(self, observation):
        if self.prev_obs is None and self.prev2_obs is None:
            self.prev_obs = observation
            return observation
        elif self.prev_obs is not None and self.prev2_obs is None:
            self.prev2_obs = self.prev_obs
            self.prev_obs = observation
            return self.prev2_obs
        else:
            tmp = self.prev2_obs
            self.prev2_obs = self.prev_obs
            self.prev_obs = observation
...
class ClipImageWrapper(gym.ObservationWrapper):
    def __init__(self, env=None, top_margin_divider=3):
        ...
        self.top_margin = 210 #top_margin
        img_height = img_height - self.top_margin
    ...
    def observation(self, observation):
        return observation[self.top_margin:, :]
...
class SegmentationWrapper(gym.ObservationWrapper):
    def __init__(self, env=None, model_dir="/home/jovyan/work/duckietown-segmentation/segmentation/models",
                 model_path="dabnetlr=0.0005optim=RMSpropepoch=15D.pth.tar", num_classes=5, height=480, width=640,
...
class CannyDetectorWrapper(gym.ObservationWrapper):
    # Need to use after resize
    def __init__(self, env):
        super(CannyDetectorWrapper, self).__init__(env)
    ...
    def observation(self, obs):
        ...
        # obs = np.array(image.filter(ImageFilter.GaussianBlur(5)))
        obs = auto_canny(obs)
        contours, _ = cv2.findContours(obs, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
        print("Count of Contours  = " + str(len(contours)))
        cv2.drawContours(obs, contours, -1, (255, 255, 255), 6)
...
class HSVHSLWrapper(gym.ObservationWrapper):
    def __init__(self, env):
        super(HSVHSLWrapper, self).__init__(env)
        self.colors = {
            YELLOW: {
                MIN: np.array([10, 50, 70]),
                MAX: np.array([35, 255, 255])
            },
            WHITE: {
                MIN: np.array([0, 144, 0]),
                MAX: np.array([179, 228, 255])
            }
        }

    def observation(self, observation):
        hsv = cv2.cvtColor(observation, cv2.COLOR_RGB2HSV)
        hls = cv2.cvtColor(observation, cv2.COLOR_RGB2HLS)
        masks = []
        yellow_params = self.colors[YELLOW]
        mask_y = cv2.inRange(hsv, yellow_params[MIN], yellow_params[MAX])
        masks.append(mask_y)
        white_params = self.colors[WHITE]
        mask_w = cv2.inRange(hls, white_params[MIN], white_params[MAX])
        masks.append(mask_w)
        masks = cv2.add(*masks)

        res = cv2.bitwise_and(observation, observation, mask=masks)
        return res